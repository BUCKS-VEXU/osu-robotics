...
const TeamRoller = ({ teams }: TeamRollerProps) => {
  ...
  useEffect(() => {
    ...
    if (!isHovered) {
      if (!displayAwards) setHoverOffCountdown(true);
      timer = setInterval(() => {
        setIsHovered(false);
  ...
  return (
    <div
      ...
      style={
        hoverOffCountdown
          ? hoverOffCountdownStyle
          : { backgroundColor: 'rgba(228, 151, 151, 0.459)' }
      }
      ...
      {showAwardsDiv && (
        <div className="awards" style={displayAwards ? mountedStyle : unmountedStyle}>
          <div className="award-item">
            Tournament Champion: <SlotCounter value={displayedTeam.tournamentChampion} />
          </div>
          <div className="award-item">
            Excellence: <SlotCounter value={displayedTeam.excellence} />
          </div>
          <div className="award-item">
            Design: <SlotCounter value={displayedTeam.design} />
          </div>
          <div className="award-item">
            Skills Champion: <SlotCounter value={displayedTeam.skillsChampion} />
          </div>
          <div className="award-item">
            Other Awards: <SlotCounter value={displayedTeam.otherAwards} />
          </div>
        </div>
      )}
    </div>
