...
export default function ActiveSessionsBoard() {
  ...
  if (err) return <Card><h3>Currently Checked In</h3><div style={{ color: "var(--danger)" }}>{err}</div></Card>;
  if (!data) return <Card><h3>Currently Checked In</h3><div>Loading…</div></Card>;
  if (data.length === 0) return <Card><h3>Currently Checked In</h3><div style={{ color: "var(--muted)" }}>No one is currently checked in.</div></Card>;
                ...
                <li key={ci.id} className="active-row">
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                      ...
                      <span className="active-row__meta" title={new Date(ci.since).toLocaleString()}>
                        {rel(new Date(ci.since))}
                      </span>
                    </div>
                    {ci.note && <div style={{ color: "var(--muted)", fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ci.note}</div>}
                  </div>
                </li>
              ))}
          </ul>
...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <div style={{
      background: "var(--elev)", border: "1px solid var(--border)", borderRadius: 16,
      boxShadow: "0 8px 24px rgb(0 0 0 / 6%)", padding: 16
    }}>
      {children}
    </div>
  );
...
function Badge({ children }: { children: React.ReactNode }) {
  return <span style={{ background: "var(--chip)", color: "var(--fg)", borderRadius: 999, padding: "2px 8px", fontSize: 12 }}>{children}</span>;
}
...
function Avatar({ url, name }: { url?: string | null; name: string }) {
  ...
  return (
    <div style={{
      width: 32, height: 32, borderRadius: "50%", background: "var(--surface)", display: "grid", placeItems: "center",
