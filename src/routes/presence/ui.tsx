...
export function Spinner({ size=18 }: { size?: number }) {
  const id = useId();
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" role="status"
         aria-labelledby={id} style={{ animation: "spin 1s linear infinite" }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        border:"1px solid rgba(0,0,0,.06)", fontWeight:600,
      ...
      onMouseEnter={(e)=>{ if(!rest.disabled && !busy) (e.currentTarget as HTMLButtonElement).style.background = base.hover; }}
      onMouseLeave={(e)=>{ if(!rest.disabled && !busy) (e.currentTarget as HTMLButtonElement).style.background = base.bg; }}
    >
...
export function Card({ children }: { children: React.ReactNode }) {
  return (
    <div style={{
      width:"100%", maxWidth:520, margin:"min(8vw,48px) auto",
...
export function Banner({ tone="info", children }:{ tone?: "info"|"ok"|"warn"|"err", children: React.ReactNode }) {
  const map = {
  ...
  return (
    <div style={{ background:map.bg, color:map.color, padding:"12px 14px", borderRadius:12, border:"1px solid rgba(0,0,0,.06)" }}>
      {children}
    </div>
  );
}
