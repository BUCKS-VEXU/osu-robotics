...
const Carousel = ({ children, slideOptions, autoSlide }: CarouselProps) => {
  ...
  const shouldAutoSlide = () => {
    if (autoSlide === undefined) return false;

    if (autoSlide === true) return true;

    if (autoSlide?.play !== undefined) return autoSlide?.play;
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      if (autoSlide === true || autoSlidePlay) {
        if (!isTransitioning) {
          if (autoSlideDirection === 'right') handleNext();
          else handlePrevious();
        }
      }
  ...
  return (
    <div className="Carousel">
      ...
      <button className="control prev" disabled={isTransitioning} onClick={handlePrevious}>
        &#x276E;
      </button>
      <button className="control next" disabled={isTransitioning} onClick={handleNext}>
        &#x276F;
      </button>
    </div>
  );
