...
const Timeline = () => {
  ...
  useEffect(() => {
    ...
    const selectObs = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          const idx = eventRefs.current.indexOf(entry.target as HTMLDivElement);
          if (idx < 0) return;
        ...
        const next = visibleSet.size > 0 ? Math.min(...visibleSet) : -1;
    ...
    <div className="Timeline">
      {events.map((event, index) => (
        ...
        >
          ...
          <div ref={(el) => (eventRefs.current[index] = el)} className="event-rails">
